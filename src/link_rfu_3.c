...
// INCGFX_U16/U32 (source PNG path + explicit output extension) is a
// pokeemerald preproc-tool feature pokeruby's own preproc doesn't have;
// using the standard INCBIN macros with the compiled paths directly instead.
...
static const struct CompressedSpriteSheet sWirelessStatusIndicatorSpriteSheet = {
    (const u8 *)sWirelessLinkIconPic, 0x0380, TAG_GFX_STATUS_INDICATOR
};
...
void LoadWirelessStatusIndicatorSpriteGfx(void)
{
    if (GetSpriteTileStartByTag(sWirelessStatusIndicatorSpriteSheet.tag) == 0xFFFF)
        LoadCompressedObjectPic(&sWirelessStatusIndicatorSpriteSheet); // pokeruby's name for Emerald's LoadCompressedSpriteSheet
...
// CopyTrainerRecord/NameIsNotEmpty/SaveLinkTrainerNames/PlayerHasMetTrainerBefore/
// WipeTrainerNameRecords all operate on gSaveBlock1Ptr->trainerNameRecords, a
// persistent "trainers you've met over wireless" history that doesn't exist in
// pokeruby's save block yet -- adding it is a save-format change deserving its
// own decision, not something to fold in silently while working on the RFU
// connection layer. Nothing ported so far calls these (they're only used by
// union_room.c, which isn't ported), so stub them out until the Trainer Card /
// Union Room registry feature is actually being built.
