...
#if RFU_USER_NAME_LENGTH < (OT_NAME_LENGTH + 1)
...
u16 rfu_initializeAPI(u32 *APIBuffer, u16 buffByteSize, IntrFunc *sioIntrTable_p, bool8 copyInterruptToRam)
{
    ...
    // is in EWRAM?
    if (((uintptr_t)APIBuffer & 0xF000000) == EWRAM && copyInterruptToRam)
...
u16 rfu_MBOOT_CHILD_inheritanceLinkStatus(void)
{
    ...
    while (*s1 != '\0')
        if (*s1++ != *s2++)
            return 1;
    mb_buff_iwram_p = (u16 *)IWRAM;

    // The size of struct RfuLinkStatus is 180
    checksum = 0;
    for (i = 0; i < 180/2; ++i)
        checksum += *mb_buff_iwram_p++;
    if (checksum != *(u16 *)(IWRAM + 0xFA))
        return 1;
    CpuCopy16((u16 *)IWRAM, gRfuLinkStatus, sizeof(struct RfuLinkStatus));
