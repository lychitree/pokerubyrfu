...
// Sized for the ROM-resident interrupt handler variant (copyInterruptToRam ==
// FALSE, see InitRFUAPI below) rather than Emerald's RFU_API_BUFF_SIZE_RAM --
// smaller, and IWRAM has very little headroom on this port (see CLAUDE.md).
COMMON_DATA u32 gRfuAPIBuffer[RFU_API_BUFF_SIZE_ROM / 4] = {0};
// Emerald tags this COMMON_DATA (IWRAM), but at 0xcf4 (3316) bytes it alone
// overflows pokeruby's much tighter IWRAM budget (see CLAUDE.md) -- moved to
// EWRAM instead, which has real headroom. No functional difference; nothing
// about gRfu's contents requires IWRAM residency or fast access.
...
void InitRFU(void)
{
    // Serial=0, Timer3=1 in pokeruby's interrupt table (not 1/2 -- see CLAUDE.md).
...
{
    // Emerald calls this with copyInterruptToRam=TRUE and indexes gIntrTable[1]/[2]
    // (its Serial/Timer3 slots). pokeruby doesn't need librfu_intr.c copied to
    // IWRAM (see CLAUDE.md milestone 0) and its interrupt table is offset by
    // one slot (Serial=0, Timer3=1, not 1/2) -- both adjusted here to match
    // the rest of this port.
    if (!rfu_initializeAPI((void *)gRfuAPIBuffer, sizeof(gRfuAPIBuffer), &gIntrTable[0], FALSE))
    {
        gLinkType = 0;
